...
let image = CommandLine.arguments.count > 1 ? CommandLine.arguments[1] : "https://storage.googleapis.com/api4ai-static/samples/alco-rec-1.jpg"
var httpBody: Data;
if (image.contains("://")) {
    // POST image via URL.
    ...
    if (error != nil) {
        ...
        do {
            ...
            if (status["code"] == "ok") {
                ...
                let labels = entity["array"] as! [[String:String]]
                for label in labels {
                    let drink = label["drink"]
                    print("\(drink ?? "unknown"):")
                    for prop in label {
                        if (prop.key != "drink") {
